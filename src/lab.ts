...
async function boot() {
  ...
  document.querySelector('#app')!.innerHTML=`
    <header class="brand">VELOCITY<span> / </span>ROGUE<small>01 — NEON COAST / DRIVING LAB</small></header>
    ...
    <details id="dev"><summary>TELEMETRIA / TUNING</summary><pre id="debug"></pre><div id="tuning"></div><button id="defaults">Ripristina tuning</button><label class="setting"><input type="checkbox" id="shake" checked> Camera stabile</label></details>
    <div id="overlay"><article><p class="eyebrow">VELOCITY ROGUE · PROTOTIPO M1</p><h1>La traiettoria<br>la scegli tu.</h1><p>Una macchina. Una pista. Trova il limite.<br>Inizia la deriva con il freno a mano, poi rilascialo e controlla il gas per generare Flow.</p><button id="start">ENTRA IN PISTA <span>↗</span></button><p class="hint">Controller: RT accelera · LT frena · stick sterza<br>A deriva · B/RB boost · Y recupera · Start pausa</p></article></div>`;
  ...
  function showOverlay(title:string,description:string,button:string) {
    $('overlay').innerHTML=`<article><p class="eyebrow">DRIVING LAB / NEON COAST</p><h1>${title}</h1><p>${description}</p><button id="resume">${button} <span>↗</span></button><button id="restart" class="secondary">Ricomincia la sessione</button></article>`;
  ...
  function pause() {
    if(state==='driving'){state='paused';showOverlay('Un respiro.','La sessione è in pausa. Tuning e recuperi restano validi.','RIPRENDI');}
  ...
  const ranges:[keyof typeof tuning,string,number,number,number][]=[['engine','Spinta motore',8000,22000,500],['grip','Grip laterale',3,12,.1],['driftGrip','Grip in deriva',.5,4,.1],['steer','Sterzo lento',.5,1.6,.05],['highSpeedSteer','Sterzo veloce',.3,1,.05]];
  ...
  renderer.setAnimationLoop(()=>{
    ...
    $('cue').textContent=vehicle.boosting?'FLOW → BOOST':vehicle.drifting?'DERIVA PULITA / +FLOW':!vehicle.grounded && state==='driving'?'IN VOLO':vehicle.integrity<30?'INTEGRITÀ CRITICA':vehicle.speed>5?'CERCA L’APICE. APRI IL GAS.':'TROVA IL TUO RITMO';
