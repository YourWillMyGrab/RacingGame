...
export const UPGRADES:readonly Upgrade[]=[
  {id:'tire-smoke',name:'Reattore di deriva',rarity:'Common',description:'Le derive controllate generano il doppio del Flow.',tags:['DERIVA','FLOW'],stats:{driftFlow:2}},
  {id:'late-apex',name:'Ultimo apice',rarity:'Common',description:'Uscire pulito da una deriva regala 1,5 s di spinta motore +60%.',tags:['DERIVA','POTENZA'],hook:'driftExit',effect:({car})=>{car.surge=1.5;}},
  ...
  {id:'armor',name:'Naso rinforzato',rarity:'Uncommon',description:'Massa +40%, danni da impatto dimezzati. La macchina pesa di più in accelerazione.',tags:['IMPATTO','INTEGRITÀ'],stats:{mass:1.4,damageScale:.5}},
  {id:'capacitor',name:'Ricircolo',rarity:'Uncommon',description:'Il boost consuma il 45% di Flow in meno.',tags:['FLOW','BOOST'],stats:{flowDrain:.55}},
  {id:'landing',name:'Carica aerea',rarity:'Uncommon',description:'Dopo oltre 0,3 s in aria, l’atterraggio regala 15 Flow.',tags:['ARIA','FLOW'],hook:'landing',effect:({car})=>{car.flow=Math.min(100,car.flow+15);}},
  ...
  {id:'drift-bank',name:'Banca laterale',rarity:'Rare',description:'Un’uscita pulita dalla deriva restituisce 12 Flow.',tags:['DERIVA','FLOW'],hook:'driftExit',effect:({car})=>{car.flow=Math.min(100,car.flow+12);}},
  {id:'overdrive',name:'Sovralimentazione',rarity:'Rare',description:'Forza del boost +80%, consumo Flow +30%.',tags:['BOOST','POTENZA'],stats:{boostForce:1.8,flowDrain:1.3}},
  {id:'afterburn',name:'Ultima fiamma',rarity:'Epic',description:'Usare il boost con meno di 25 Flow raddoppia la spinta motore.',tags:['BOOST','RISCHIO'],hook:'boost',effect:({car})=>{if(car.flow<25)car.power*=2;}},
  {id:'repair-drift',name:'Officina in curva',rarity:'Epic',description:'Le derive controllate riparano 3 integrità al secondo.',tags:['DERIVA','INTEGRITÀ'],hook:'drift',effect:({car,dt})=>{car.integrity=Math.min(100,car.integrity+dt*3);}},
  {id:'perpetual',name:'Moto perpetuo',rarity:'Legendary',description:'Sopra 90 km/h guadagni 8 Flow al secondo, anche in boost.',tags:['VELOCITÀ','FLOW'],hook:'tick',effect:({car,dt})=>{if(car.speed>25)car.flow=Math.min(100,car.flow+8*dt);}},
  ...
  {id:'blood-fuel',name:'Sangue nel motore',rarity:'Cursed',description:'Puoi usare il boost anche senza Flow.',downside:'A Flow esaurito, il boost consuma 8 integrità al secondo e può distruggerti.',tags:['MALEDIZIONE','BOOST'],hook:'tick',effect:({car,dt,input})=>{if(input.boost&&input.throttle>.1&&car.grounded&&car.speed>2&&car.flow<=car.settings.flowDrain*dt&&car.integrity>0){car.flow+=car.settings.flowDrain*dt;car.integrity=Math.max(0,car.integrity-8*dt);}}},
];
