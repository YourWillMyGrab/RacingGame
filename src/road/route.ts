...
export class ModularRoute implements DrivableRoute {
  ...
  constructor(seed: string, count=24) {
    ...
    const rng=randomStream(this.seed,'road-v1');
    let socket:Socket={x:64,y:0,z:95,yaw:0,grade:0,width:20};
    for(let i=0;i<count;i++) {
      const previous=this.chunks.at(-1)?.definition;
      const options=MODULES.filter(m=>m.category!=='start'&&m.category!=='finish'&&m.id!==previous?.id
      ...
      for(let j=0;j<=n;j++) {
        ...
        points.push({x,y:socket.y+m.rise*Math.sin(Math.PI*u)**2,z,yaw:socket.yaw+m.turn*u+m.wave*Math.sin(u*Math.PI*2),s:start+m.length*u,width:m.width});
      }
  ...
  pointAt(s: number): RoadPoint {
    ...
    return {x:a.x+(b.x-a.x)*t,y:a.y+(b.y-a.y)*t,z:a.z+(b.z-a.z)*t,yaw:a.yaw+(b.yaw-a.yaw)*t,s,width:a.width};
  }
  nearest(x: number,z: number,hint?: number): RoadAnchor {
