...
function define(id: string, category: RoadModule['category'], length: number, turn=0, wave=0, rise=0): RoadModule {
  const width=20, curvature=(Math.abs(turn)+Math.abs(wave)*2*Math.PI)/length;
  const recommendedSpeed=curvature>.006 ? 25 : 35;
