...
- M5.2 browser gate is pending: this task passed 37 automatic tests/build/stress, but the current environment has no system Chrome, the standard browser download times out and alternate Chromium 153 crashes with SIGSEGV before loading the game. No M5.2 screenshot/layout or full browser run is validated yet. Run all browser scripts on working Chrome; use BROWSER_CONFIG for an alternative Playwright launch configuration.
