...
test('six physical racers stay on grid during countdown, race to finish and keep results',()=>{
 ...
 try {
  ...
  for(let i=0;i<8000 && race.order.length<6;i++){
   ...
   race.step({throttle:.92,brake:0,steer:Math.max(-1,Math.min(1,e*2.4)),handbrake:false,boost:false},STEP);
  }
  ...
  const order=JSON.stringify(race.order);for(let i=0;i<30;i++)race.step(IDLE,STEP);assert.equal(JSON.stringify(race.order),order);assert.ok(race.position>=1&&race.position<=6);
