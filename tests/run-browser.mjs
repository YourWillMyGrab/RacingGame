...
try {
 ...
 for(let event=0;event<3;event++){
  const initial=await page.evaluate(()=>window.__roadGame);
  ...
  if(initial.race.kind==='time-attack') {
   ...
   await page.screenshot({path:`test-results/time-attack-start-${event}.png`});
  ...
  if(snapshot.race.kind==='time-attack') {
   ...
   await page.screenshot({path:`test-results/time-attack-result-${event}.png`});
  }
  ...
  if(event<2){assert.ok(recoveredInFork);assert.equal(snapshot.run.routeChoices[event].branch,branch);assert.equal(snapshot.run.nextProfile,event===0?'technical':'speed');
   ...
   snapshot=await page.evaluate(()=>window.__roadGame);assert.equal(snapshot.integrity,Math.min(100,Math.max(1,integrity+(snapshot.run.results[event].position<=3?8:-8))));assert.equal(snapshot.run.owned.length,event+1);assert.equal(snapshot.run.event,event+1);assert.equal(snapshot.state,'driving');assert.ok(snapshot.race.countdown>2);assert.equal(snapshot.run.profile,event===0?'technical':'speed');
  }else{assert.equal(snapshot.run.phase,'complete');await page.screenshot({path:'test-results/run-complete.png'});}
 }
 await page.click('#restart');const reset=await page.evaluate(()=>window.__roadGame);assert.deepEqual(reset.run.results,[]);assert.equal(reset.race.kind,'road-race');assert.deepEqual(reset.run.owned,[]);assert.deepEqual(reset.run.routeChoices,[]);assert.equal(reset.run.profile,'balanced');assert.equal(reset.run.event,0);assert.equal(reset.integrity,100);assert.equal(reset.flow,25);assert.deepEqual(errors,[]);console.log('PASS mixed run, Time Attack targets/pause/results, both physical arms, recovery and next-event profiles, keyboard/controller rewards, victory and clean reset');
