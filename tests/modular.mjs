...
await page.evaluate(async()=>{
  ...
  function drive(){const v=window.__roadGame;const target=route.pointAt(v.progress+12+v.speed*.22);let error=Math.atan2(-(target.x-v.position.x),-(target.z-v.position.z))-v.yaw;error=Math.atan2(Math.sin(error),Math.cos(error));const steer=Math.max(-1,Math.min(1,error*2.4));pad.axes[0]=-Math.sign(steer)*(.12+Math.abs(steer)*.88);pad.buttons[7]={pressed:true,value:.9};if(v.state==='driving')requestAnimationFrame(drive);else pad.buttons[7]={pressed:false,value:0};}requestAnimationFrame(drive);
});
