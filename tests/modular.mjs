...
await page.evaluate(async()=>{
  ...
  function drive(){const v=window.__roadGame;const target=route.pointAt(v.progress+6+v.speed*.32);let error=Math.atan2(-(target.x-v.position.x),-(target.z-v.position.z))-v.yaw;error=Math.atan2(Math.sin(error),Math.cos(error));const steer=Math.max(-1,Math.min(1,error*3));pad.axes[0]=-Math.sign(steer)*(.12+Math.abs(steer)*.88);const limit=route.speedAt(v.progress)*.96;pad.buttons[7]={pressed:v.speed<limit,value:v.speed<limit?.94:0};pad.buttons[6]={pressed:v.speed>limit+.7,value:v.speed>limit+.7?.65:0};if(v.state==='driving')requestAnimationFrame(drive);else pad.buttons[7]={pressed:false,value:0};}requestAnimationFrame(drive);
});
